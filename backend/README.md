...
- **Postgres / `DATABASE_URL`:** Hikari ожидает **`jdbc:postgresql://user:pass@host:port/db`** (после `postgresql` обязательно **`//`**, иначе драйвер воспринимает `user:pass@host` как одно имя хоста и соединение падает до старта HTTP). Надёжный шаблон в Railway: **`jdbc:postgresql://${{Postgres.PGUSER}}:${{Postgres.PGPASSWORD}}@${{Postgres.PGHOST}}:${{Postgres.PGPORT}}/${{Postgres.PGDATABASE}}`** (имя сервиса `Postgres` замените при необходимости; пароль со спецсимволами — URL-encode). Альтернатива: сырое **`postgresql://…`** или **`jdbc:postgresql://…`** — при старте URL нормализуется (`JdbcDatabaseUrlEnvironmentPostProcessor`), в т.ч. пропущенный `//`, дубль схемы и для публичного прокси **`*.rlwy.net`** добавляется **`?sslmode=require`**, если параметра ещё нет (внутренний `*.railway.internal` не трогаем).
